...
# ═══════════════════════════════════════════════════════════════════
# Model list untuk base_url LOKAL (9Router/Free-All / proxy OpenAI-compatible)
# Dipakai kalau base_url != OpenRouter. Model ID di bawah ini yang umum
# tersedia di proxy lokal (9Router combo Free-All, tokenin myt/, dll).
# ═══════════════════════════════════════════════════════════════════

LOCAL_MODELS = [
    # ✅ TERVERIFIKASI JALAN (2026-08-29, test live) — ditaruh paling atas
    'L',  # combo default 9Router — selalu jalan, free
    'or/minimax/minimax-m3:free',  # OpenRouter free tier — jalan
    'bai/deepseek-v4-flash',       # api.b.ai — jalan (sisa kuota)
    'bai/glm-5.3-flash',           # api.b.ai — jalan
    'bai/qwen3.8-flash',           # api.b.ai
    'bai/gemini-3.6-flash',        # api.b.ai
    'bai/mimo-v2.5',               # api.b.ai
    'bai/deepseek-v4-flash-vision-exp',  # api.b.ai
    'or/openrouter/free',          # OpenRouter free router
    'tokenrouter/openai/gpt-4o-mini',    # tokenrouter
    'tokenrouter/openai/gpt-5-mini',
    'tokenrouter/moonshotai/kimi-k3-free',
    'tokenrouter/openai/gpt-5.4-mini',
    'ftf/deepseek-v4-flash',       # freetokenfaucet
    'ftf/gpt-5.6-luna',
    'ftf/claude-sonnet-5',
    'ftf/glm-5',
    ...
    'bai/gpt-5.4-mini',
    'bai/gpt-5-nano',
    'tabi/claude-opus-5',          # tabitoken
    'tabi/claude-opus-4-8',
    'gcli/grok-4.6',               # grok cli
    'xai/grok-4',
    'kimi/kimi-k3',                # kimi official
    'kimi/kimi-k2.5',
    'gemini/gemini-3.6-flash',     # google official
    'gemini/gemini-3.5-flash',
    'ds/deepseek-v4-flash',        # deepseek official
    'ds/deepseek-chat',
    'groq/meta-llama/llama-4-maverick-17b-128e-instruct',  # groq
    'groq/qwen/qwen3-32b',
    # ⚠️ DIBATASI (kuota bulanan habis / perlu whitelist key) — cadangan
    'kr/claude-opus-5', 'kr/claude-sonnet-5', 'kr/deepseek-3.2',
    'kr/gpt-5.6-sol', 'kr/glm-5', 'kr/minimax-m2.5',
    'myt/gpt-5-mini', 'myt/gpt-5.4-mini', 'myt/qwen3.6-flash',
    'myt/deepseek-v4-flash', 'myt/kimi-k2.5', 'myt/MiniMax-M2.5',
    'myt/gemini-3.5-flash-free', 'myt/gpt-5.6-luna-free', 'myt/qwen3.8-max-free',
    'myt/myt/deepseek-v4-flash', 'myt/myt/gemini-3.5-flash-free',
    'gs/gpt-5', 'gs/gpt-5.4', 'gs/claude-sonnet-5', 'gs/claude-opus-5',
    'gs/deep-seek-v4-flash', 'gs/kimi-k3', 'gs/glm-5p2', 'gs/grok-4.6',
    'oc/deepseek-v4-flash-free', 'oc/mimo-v2.5-free', 'oc/x-preview-f-free',
    'cag/deepseek-v4-flash', 'cag/glm-5.3-flash',
    'gg/models/gemini-3.6-flash', 'gg/models/gemini-3.5-flash',
    # fallback umum (OpenRouter style — kalau proxy bukan 9Router)
    ...
    'qwen/qwen-2.5-72b-instruct', 'nousresearch/hermes-3-llama-3.1-70b',
]

# Auto-fetch model list dari 9Router / proxy lokal — override LOCAL_MODELS
def _fetch_models(base_url, timeout=10):
    """Coba fetch /v1/models dari proxy lokal, return list model ID atau None."""
    try:
...
def race_models(query, tier="standard", api_key=None, system_prompt=None,
                ...
                jailbreak_system=None, prefill=None, base_url=None,
    ...
    Args:
        ...
        api_key: OpenRouter API key (defaults to OPENROUTER_API_KEY env var,
                 or model.api_key dari config.yaml untuk proxy lokal)
        ...
        base_url: Override API base URL. Auto-detect dari config.yaml kalau None
                  (9Router lokal 127.0.0.1:20128/v1 dideteksi otomatis).
        models: Override model list (list of str). Default: ULTRAPLINIAN_MODELS
                untuk OpenRouter, LOCAL_MODELS untuk proxy lokal.
    ...
    if not base_url:
        # coba baca dari config.yaml (model.base_url / model.api_key)
        try:
            ...
            if os.path.exists(_cfg_path):
                ...
                if isinstance(_mc, dict):
                    base_url = _mc.get("base_url") or _mc.get("api_base")
        except Exception:
            pass
    if not base_url:
        base_url = "https://openrouter.ai/api/v1"
    
    is_local = ("127.0.0.1" in base_url or "localhost" in base_url or "20128" in base_url)
    
    if not api_key:
        # pakai _get_api_key dari auto_jailbreak (baca env + .env + config)
        try:
            from auto_jailbreak import _get_api_key, _load_env_file
        except ImportError:
            try:
                import importlib.util
                _aj = os.path.join(os.path.dirname(__file__), "auto_jailbreak.py")
                _spec = importlib.util.spec_from_file_location("_aj", _aj)
                _mod = importlib.util.module_from_spec(_spec)
                _spec.loader.exec_module(_mod)
                _get_api_key = _mod._get_api_key
            except Exception:
                _get_api_key = lambda b: os.getenv("OPENROUTER_API_KEY", "")
        api_key = _get_api_key(base_url)
    if not api_key:
        # proxy lokal sering gak butuh key — coba key dummy kalau lokal
        if is_local:
            api_key = "local"
        else:
            raise ValueError("No API key. Set OPENROUTER_API_KEY, config.yaml model.api_key, atau pass api_key=")
    ...
    if models:
        ...
        # coba fetch daftar model asli dari proxy — filter yang chat-relevant
        fetched = _fetch_models(base_url)
        if fetched:
            # pilih model yang relevan (bukan image/video/embedding, tanpa -thinking/-agentic dup)
            import re as _re
            skip = _re.compile(r"(image|video|tts|stt|embed|rerank|moderation|thinking|agentic)", _re.I)
            chat_models = [m for m in fetched if not skip.search(m) and "/" in m]
            model_list = chat_models or LOCAL_MODELS
        else:
            model_list = LOCAL_MODELS
    else:
        model_list = ULTRAPLINIAN_MODELS
    
    model_count = TIER_SIZES.get(tier, TIER_SIZES['standard'])
    model_list = model_list[:model_count]
    
    if verbose:
        print(f"[RACE] base_url={base_url} | models={len(model_list)} | tier={tier}")
