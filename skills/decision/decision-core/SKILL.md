# Decision Core

## Purpose

Keputusan kompleks & trade-off.

## Kapan Digunakan

- Mengoordinasikan skill di dalam layer **Planning / Decision Intelligence**.
- Sebagai entry point sebelum sub-skill dijalankan.
- Saat perlu menentukan alur antar-step dalam domain ini.

## Workflow

Discover
→ Select
→ Delegate
→ Track
→ Verify
→ Report

## Rules

- Frame → Options → Criteria → Evaluate → Decide → Review
- Pisahkan goal dari wording.
- Bukti > asumsi.
- Jangan asumsikan sukses tanpa verifikasi.
- Catat keputusan & hasil agar dapat diaudit.

## Integration

- Berjalan di bawah `decision-core`.
- Berkoordinasi dengan `policy` (guardrails) & `state` (checkpoint) bila relevan.
- Hasilnya dapat dikonsumsi `analytics` & `learning`.

## Cross-Layer

- Guardrails: `policy/*`
- State: `state/*`
- Learning: `learning/*`
- Analytics: `analytics/*`

## Layer

Planning / Decision Intelligence
